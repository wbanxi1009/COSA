...
usage() {
  cat <<'EOF'
Usage: bash scripts/train_eval_cosa_lengths.sh [options]
...
Examples:
  ...
  LABEL_LEN=96 bash scripts/train_eval_cosa_lengths.sh --models PatchTST --datasets weather --input-lens 336 --pred-lens 720
EOF
}
  ...
  for DATASET in "${DATASETS[@]}"; do
    for INPUT_LEN in "${INPUT_LENS[@]}"; do
      for PRED_LEN in "${PRED_LENS[@]}"; do
        ...
        COSA_EXPERIMENT_DIR="${COSA_RESULT_ROOT}/${MODEL}/${DATASET}/${RUN_ID}"
        CHECKPOINT_PATH="${CHECKPOINT_DIR}checkpoint_best.pth"
        BASELINE_RESULT_PATH="${BASELINE_EXPERIMENT_DIR}/test.txt"
        COSA_COMPLETE_PATH="${COSA_EXPERIMENT_DIR}/cosa_complete"
