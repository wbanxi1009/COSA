#!/usr/bin/env python3
"""Collect PETSA metrics produced by the length-sweep script.

Expected layout:
  results/PETSA/<model>/<dataset>/input_<input_len>_pred_<pred_len>/
    petsa_output.log
    petsa_complete
"""
...
def parse_final_result(path: Path) -> dict[str, object]:
    ...
    for position in reversed([index for index, char in enumerate(content) if char == "{"]):
        try:
            ...
            continue
        if isinstance(result, dict) and result.get("model") == "PETSA":
            return result
    raise ValueError("no complete PETSA JSON result found")


def collect(results_root: Path) -> tuple[list[dict[str, object]], list[dict[str, str]]]:
    rows = []
    errors = []
    for log_path in results_root.glob("*/*/input_*_pred_*/petsa_output.log"):
        ...
        try:
            ...
            row = {
                ...
                "status": "complete" if (log_path.parent / "petsa_complete").is_file() else "missing_completion_marker",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect PETSA length-sweep metrics from output logs.")
    parser.add_argument("--results-root", type=Path, default=Path("results/PETSA"))
    parser.add_argument("--format", choices=("markdown", "json"), default="markdown")
    parser.add_argument("--output", type=Path, help="Write the collected table or JSON to this file.")
    args = parser.parse_args()

    if not args.results_root.is_dir():
        print(f"PETSA results directory does not exist: {args.results_root}", file=sys.stderr)
        return 1
    ...
    print(f"Collected {len(rows)} PETSA result(s).", file=sys.stderr)
    for error in errors:
