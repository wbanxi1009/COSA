...
# Run a resumable PETSA sweep over models, datasets, input lengths, and
...
# PETSA configuration. PAAS dynamically sets each adaptation batch size.
BASE_LR=0.001
WEIGHT_DECAY=0.0001
GATING_INIT=0.01
RANK=16
...
# "PETSA" is required by main.py to dispatch to tta.petsa.
PETSA_RESULT_ROOT="./results/PETSA"

usage() {
  cat <<'EOF'
Usage: bash scripts/train_eval_petsa_lengths.sh [options]
...
Examples:
  bash scripts/train_eval_petsa_lengths.sh --models iTransformer --datasets ETTh1 --input-lens 96,192 --pred-lens 96,192
  LABEL_LEN=96 bash scripts/train_eval_petsa_lengths.sh --models PatchTST --datasets weather --input-lens 336 --pred-lens 720
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --models) [[ $# -ge 2 ]] || { echo "Missing value for --models" >&2; exit 1; }; parse_csv "$2" MODELS; shift 2 ;;
    --datasets) [[ $# -ge 2 ]] || { echo "Missing value for --datasets" >&2; exit 1; }; parse_csv "$2" DATASETS; shift 2 ;;
    --input-lens) [[ $# -ge 2 ]] || { echo "Missing value for --input-lens" >&2; exit 1; }; parse_csv "$2" INPUT_LENS; shift 2 ;;
    --pred-lens) [[ $# -ge 2 ]] || { echo "Missing value for --pred-lens" >&2; exit 1; }; parse_csv "$2" PRED_LENS; shift 2 ;;
    -h|--help) usage; exit 0 ;;
  ...
  for DATASET in "${DATASETS[@]}"; do
    for INPUT_LEN in "${INPUT_LENS[@]}"; do
      for PRED_LEN in "${PRED_LENS[@]}"; do
        ...
        PETSA_EXPERIMENT_DIR="${PETSA_RESULT_ROOT}/${MODEL}/${DATASET}/${RUN_ID}"
        CHECKPOINT_PATH="${CHECKPOINT_DIR}checkpoint_best.pth"
        BASELINE_RESULT_PATH="${BASELINE_EXPERIMENT_DIR}/test.txt"
        PETSA_COMPLETE_PATH="${PETSA_EXPERIMENT_DIR}/petsa_complete"
        ...
        if [[ -f "${CHECKPOINT_PATH}" ]]; then
          ...
          python main.py \
            DATA.NAME "${DATASET}" DATA.SEQ_LEN "${INPUT_LEN}" DATA.LABEL_LEN "${LABEL_LEN}" DATA.PRED_LEN "${PRED_LEN}" \
            MODEL.NAME "${MODEL}" MODEL.seq_len "${INPUT_LEN}" MODEL.label_len "${LABEL_LEN}" MODEL.pred_len "${PRED_LEN}" \
            TRAIN.ENABLE True TEST.ENABLE False TTA.ENABLE False \
        ...
        if [[ -s "${BASELINE_RESULT_PATH}" ]]; then
          ...
          python main.py \
            DATA.NAME "${DATASET}" DATA.SEQ_LEN "${INPUT_LEN}" DATA.LABEL_LEN "${LABEL_LEN}" DATA.PRED_LEN "${PRED_LEN}" \
            MODEL.NAME "${MODEL}" MODEL.seq_len "${INPUT_LEN}" MODEL.label_len "${LABEL_LEN}" MODEL.pred_len "${PRED_LEN}" \
            TRAIN.ENABLE False TEST.ENABLE True TTA.ENABLE False \
            TRAIN.CHECKPOINT_DIR "${CHECKPOINT_DIR}" RESULT_DIR "${BASELINE_RESULT_ROOT}" \
            2>&1 | tee "${BASELINE_EXPERIMENT_DIR}/baseline_output.log"
        ...
        if [[ -f "${PETSA_COMPLETE_PATH}" ]]; then
          echo "Skipping PETSA; completed result exists: ${PETSA_COMPLETE_PATH}"
        else
          mkdir -p "${PETSA_EXPERIMENT_DIR}"
          echo "Running PETSA: model=${MODEL}, dataset=${DATASET}, input_len=${INPUT_LEN}, pred_len=${PRED_LEN}"
          python main.py \
            DATA.NAME "${DATASET}" DATA.SEQ_LEN "${INPUT_LEN}" DATA.LABEL_LEN "${LABEL_LEN}" DATA.PRED_LEN "${PRED_LEN}" \
            MODEL.NAME "${MODEL}" MODEL.seq_len "${INPUT_LEN}" MODEL.label_len "${LABEL_LEN}" MODEL.pred_len "${PRED_LEN}" \
            TRAIN.ENABLE False TEST.ENABLE False TTA.ENABLE True \
            TRAIN.CHECKPOINT_DIR "${CHECKPOINT_DIR}" RESULT_DIR "${PETSA_RESULT_ROOT}" \
            TTA.SOLVER.BASE_LR "${BASE_LR}" TTA.SOLVER.WEIGHT_DECAY "${WEIGHT_DECAY}" \
            TTA.PETSA.GATING_INIT "${GATING_INIT}" TTA.PETSA.RANK "${RANK}" TTA.PETSA.LOSS_ALPHA "${LOSS_ALPHA}" \
            TTA.PETSA.PAAS "${PAAS}" TTA.PETSA.PERIOD_N "${PERIOD_N}" TTA.PETSA.STEPS "${STEPS}" \
            TTA.PETSA.ADJUST_PRED "${ADJUST_PRED}" TTA.PETSA.CALI_MODULE "${CALI_MODULE}" \
            2>&1 | tee "${PETSA_EXPERIMENT_DIR}/petsa_output.log"
          touch "${PETSA_COMPLETE_PATH}"
        fi
      done
    done
  done
