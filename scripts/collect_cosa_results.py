#!/usr/bin/env python3
"""Collect COSA-F metrics produced by the length-sweep script.

Expected layout:
  results/SIMPLE/COSA_F/<model>/<dataset>/input_<input_len>_pred_<pred_len>/
    cosa_output.log
    cosa_complete
"""
...
def parse_final_result(path: Path) -> dict[str, object]:
    ...
    # Console output precedes the final pretty-printed result. Decode JSON
    # objects from the end to avoid relying on a fixed line number.
    for position in reversed([index for index, char in enumerate(content) if char == "{"]):
        try:
            ...
            continue
        if isinstance(result, dict) and result.get("model") == "SimpleAdapter":
            return result
    raise ValueError("no complete SimpleAdapter JSON result found")


def collect(results_root: Path) -> tuple[list[dict[str, object]], list[dict[str, str]]]:
    rows = []
    errors = []
    for log_path in results_root.glob("*/*/input_*_pred_*/cosa_output.log"):
        ...
        try:
            ...
            row = {
                ...
                "status": "complete" if (log_path.parent / "cosa_complete").is_file() else "missing_completion_marker",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect COSA-F length-sweep metrics from output logs.")
    parser.add_argument("--results-root", type=Path, default=Path("results/SIMPLE/COSA_F"))
    parser.add_argument("--format", choices=("markdown", "json"), default="markdown")
    parser.add_argument("--output", type=Path, help="Write the collected table or JSON to this file.")
    args = parser.parse_args()

    if not args.results_root.is_dir():
        print(f"COSA-F results directory does not exist: {args.results_root}", file=sys.stderr)
        return 1
    ...
    print(f"Collected {len(rows)} COSA-F result(s).", file=sys.stderr)
    for error in errors:
